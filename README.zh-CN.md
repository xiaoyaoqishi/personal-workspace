...
- Trading 现已提供完全独立的“研究”子模块，页面采用与文档模块一致的资料夹树和单一阅读/编辑区，并支持跨资料夹拖拽、同资料夹排序、搜索、标签、置顶、Wiki 双向关联与研究回收站。研究阅读态的正文图片支持连续按比例放大、缩小、复位及放大后拖拽查看。富文本编辑栏能力与 Notes 文档编辑器保持一致，但代码、API 和数据表仍完全归属交易域；旧的交易作用域 Notes 数据会在运行期迁移时复制到独立研究域。
- 交易侧回收站，覆盖交易、券商和计划。
...
- 文档支持拖拽到其他文件夹，也支持在同一文件夹内调整顺序；原有移动按钮继续保留。
- 搜索、日历、日记树和历史回看能力。
