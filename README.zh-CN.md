...
- 面向交易记录的统计与分析接口，仪表盘支持按日期、交易类型、品种和券商来源筛选。
...
- Trading 现已提供完全独立的“研究”子模块，页面采用与文档模块一致的资料夹树和单一阅读/编辑区；研究和资料夹都支持拖拽移动与同级排序，资料夹还可通过拖拽调整层级，并提供搜索、标签、置顶、Wiki 双向关联与研究回收站。研究阅读态的正文图片支持连续按比例放大、缩小、复位及放大后拖拽查看。富文本编辑栏能力与 Notes 文档编辑器保持一致，但代码、API 和数据表仍完全归属交易域；旧的交易作用域 Notes 数据会在运行期迁移时复制到独立研究域。
- 交易侧回收站，覆盖交易、券商和计划。
