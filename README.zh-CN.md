...
- Trading 现已增加独立的“研究”子模块，在交易前端内嵌与 Notes 文档页一致的文档能力，同时将研究数据与主笔记模块隔离。
...
- npm
- 可选：`tmux`
- 生产部署需要 Linux 和 Docker（`docker compose`）
...
- `deploy/`：部署脚本与 Docker/Nginx 运行配置。
- `docker-compose.yml`：生产容器编排入口。
- `Dockerfile.backend` / `Dockerfile.web`：后端与 Web 运行镜像定义。
...
## 部署说明
生产部署默认采用 Docker Compose。首次部署使用 `deploy/docker-up.sh`，常规更新使用 `deploy/docker-update.sh`。运行时分为两个容器：`backend`（FastAPI）和 `web`（Nginx + portal + 前端构建产物）。`web` 容器将 `/api/*` 代理到 `backend:8000`，持久化数据通过 Docker 卷隔离（`backend_data`、`backend_uploads`）。
