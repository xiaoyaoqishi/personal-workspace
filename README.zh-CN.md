...
- `docker-compose.yml`：历史容器部署入口，已不再作为默认生产路径。
- `Dockerfile.backend` / `Dockerfile.web`：历史容器镜像定义，仅为兼容或回滚保留。
...
- `deploy/docker-up.sh` 与 `deploy/docker-update.sh`：已降级为废弃提示脚本，用于阻止误用 Docker 部署入口。
