...
## 模块地图
- `Trading`：负责交易记录、分析、复盘会话、交易计划和研究工作流。
...
- `backend/`：统一 FastAPI 后端，既包含交易域，也包含独立的账务后端域 `/api/ledger/*`。
  ...
  - `trading/`：交易域专用业务逻辑，如导入、分析、复盘和计划等。
